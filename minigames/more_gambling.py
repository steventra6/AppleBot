...
async def setup_gambling_games(bot, economy: gamble.Economy, guild_id: str):
    ...
    async def coinflip(interaction: discord.Interaction, bet: int, choice: str):
        ...
        if economy.is_on_cooldown(user_id, command_name):
            ...
            logger.debug(f"{interaction.user.name} is on cooldown for /coinflip. They must wait {remaining_time}")
        ...
        if bet <= 0 or bet > current_balance:
            logger.debug(f"{interaction.user.name} bet an invalid amount on /coinflip. Bet amount: ${bet}. Their balance: ${current_balance}")
        ...
        if winnings > 0:
            logger.debug(f"{interaction.user.name} won the coin flip and earned ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
        else:
            logger.debug(f"{interaction.user.name} lost the coin flip and lost ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
    ...
    async def roll(interaction: discord.Interaction, bet: int, guess: int):
        ...
        if economy.is_on_cooldown(user_id, command_name):
            remaining_time = economy.get_cooldown_time(user_id, command_name)
            logger.debug(f"{interaction.user.name} is on cooldown for the /roll command. They must wait {remaining_time}")
            await interaction.response.send_message(f"You're on cooldown! Try again in {remaining_time}.", ephemeral=True)
            return
        
        if bet <= 0 or bet > current_balance:
            logger.debug(f"{interaction.user.name} bet an invalid amount on /roll. Bet amount: ${bet}. Their balance: ${current_balance}")
            await interaction.response.send_message("Invalid bet amount!", ephemeral=True)
            return
        
        if guess < 1 or guess > 6:
            logger.debug(f"{interaction.user.name} guessed a number that was not between 1 and 6 for /roll")
        ...
        if winnings > 0:
            logger.debug(f"{interaction.user.name} won the dice roll and earned ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
        else:
            logger.debug(f"{interaction.user.name} lost the dice roll and lost ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
    ...
    async def lottery(interaction: discord.Interaction, bet: int):
        ...
        if economy.is_on_cooldown(user_id, command_name):
            remaining_time = economy.get_cooldown_time(user_id, command_name)
            logger.debug(f"{interaction.user.name} is on cooldown for /lottery. They must wait {remaining_time}")
            await interaction.response.send_message(f"You're on cooldown! Try again in {remaining_time}.", ephemeral=True)
            return
        ...
        if winnings > 0:
            logger.debug(f"{interaction.user.name} won the jackpot and earned ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
        else:
            logger.debug(f"{interaction.user.name} lost the jackpot and lost ${winnings}. Their new balance: ${economy.get_balance(user_id)}")
